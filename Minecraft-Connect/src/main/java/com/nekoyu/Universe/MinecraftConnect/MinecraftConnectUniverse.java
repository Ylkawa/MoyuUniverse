...
public class MinecraftConnectUniverse extends Law implements UniverseListener {
    Multimap<String, String> forwardingStructure = ArrayListMultimap.create();
    @Override
        ...
        try (FileReader fr = new FileReader("./config/Minecraft-Connect/config.yml")) {
            ...
            for (String value : forward) {
                if (Pattern.matches("[a-zA-Z0-9]+->[a-zA-Z0-9]+:[a-zA-Z0-9/]+", value)) {
                    ...
                    forwardingStructure.put(args[0].strip(), arg1[0].strip() + ":" + arg1[1].strip());
                } else {
                    logger.warn("{} 行的定义有误，正确示例：[ServerID] -> [MessageChannelID]:[SessionID]", value);
                }
            }
    ...
    public void onMessage(String ID, String message, Map args) {
        Collection<String> defineOfForward = forwardingStructure.get(ID);
        switch (message) {
            case "player_join_game":
                ...
                for (String value : defineOfForward) {
                    ...
                    MessageChannel mc = Universe.MessageChannels.get(target[0]);
                    if (mc == null) {
